...
var tanuki_progress = DialogueTracker.tanuki_progress
...
func _physics_process(delta: float) -> void:
	if player_in_area:
		...
		if Input.is_action_just_pressed("interact") and PlayerPause.playerPaused != true and dialogue_cooldown == false:
			match DialogueTracker.tanuki_progress:
				NONE:
					run_dialogue("tanuki_meeting")
					PlayerPause.playerPaused = true
					DialogueTracker.tanuki_progress = MET
				MET:
					run_dialogue("tanuki_met")
					PlayerPause.playerPaused = true
