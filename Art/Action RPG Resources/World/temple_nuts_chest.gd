...
func _physics_process(delta: float) -> void:
	if player_in_area and DialogueTracker.temple_nuts_chest_progress == NONE:
...
func DialogicSignal(arg: String):
	if arg == "exit_nuts_chest":
		PlayerPause.playerPaused = false
		DialogueTracker.temple_nuts_chest_progress = MET
		Wallet.wallet += 25
